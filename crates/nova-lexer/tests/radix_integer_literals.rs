...
fn lexes_binary_octal_and_hexadecimal_magnitudes() {
    let source = source("0b1010_1010 0B10101010 0o52 0O52 0x2a 0X2A 0x7fff_ffff_ffff_ffff 0x8000_0000_0000_0000");
...
fn rejects_invalid_digits_and_separator_placement_as_one_literal() {
    for text in ["0b", "0b_1", "0b102", "0o8", "0xg", "0x1__0", "0x1_", "123abc"] {
        let output = lex(&source(text));
        assert_eq!(output.diagnostics.len(), 1, "{text}: {:?}", output.diagnostics);
