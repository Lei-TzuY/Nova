...
#[test]
fn int_sign_predicates_evaluate_their_operand_once() {
    let source = r#"
fn main() -> Int {
    var x: Int = -2;
    let negative = Int::is_negative({ x = x + 1; x });
    if negative { x + 43 } else { 0 }
}
"#;
...
#[test]
fn int_sign_predicates_reject_non_int_payloads_and_missing_payloads() {
    for source in [
        "fn main() -> Bool { Int::is_negative(true) }",
        "fn main() -> Bool { Int::is_zero(\"0\") }",
        "fn main() -> Bool { Int::is_positive }",
    ] {
