...
def replace(path, old, new, count=1):
    ...
    assert found == count, f"{path}: expected {count} matches, found {found}"
    p.write_text(text.replace(old, new, count))


# AST: generic parameters belong to top-level functions.
replace(
...
# Parser: fn name<T, U>(...). Less/Greater are already lexical tokens.
replace(
    ...
    '''        let name = self.parse_name("after `fn`")?;\n        let type_parameters = self.parse_type_parameters()?;\n        self.expect(TokenKind::LeftParen, "after the function name or type parameters")?;\n        let parameters = self.parse_parameters()?;''',
)
replace(
    "crates/nova-parser/src/parser.rs",
    '''        Some(Function {\n            name,\n            parameters,''',
...
replace(
    ...
    '''                    .map(|parameter| {\n                        self.resolve_type_ref_with_parameters(parameter, type_parameters)\n                    })\n                    .collect(),\n                return_type: Box::new(self.resolve_type_ref_with_parameters(\n                    return_type,\n                    type_parameters,\n                )),''',
)

# Replace call checker with inference + substitution. This intentionally supports recursive
# structural inference through function types but not nominal generic records/enums yet.
p = Path("crates/nova-sema/src/analyzer.rs")
text = p.read_text()
start = text.index("    fn check_call(\n")
end = text.index("    fn join_branch_types(\n", start)
replacement = r'''    fn infer_generic_argument(
        &mut self,
        actual: &Type,
        expected: &Type,
        type_parameters: &BTreeSet<String>,
...
# Unconstrained generic parameters have no equality constraint in this first slice.
replace(
    "crates/nova-sema/src/equality_rules.rs",
    '''        Type::Record(_) | Type::Never | Type::Error => false,''',
    '''        Type::Record(_) | Type::TypeParameter(_) | Type::Never | Type::Error => false,''',
)

# Constant-condition classification treats T as unknown, never as a closed primitive.
p = Path("crates/nova-sema/src/constant_condition.rs")
text = p.read_text()
assert "Type::UInt => Ok(false)," in text
p.write_text(text.replace("Type::UInt => Ok(false),", "Type::UInt | Type::TypeParameter(_) => Ok(false),", 1))

# Runtime instantiation stack: rigid T is resolved during a concrete function invocation.
replace(
...
text = p.read_text()
start = text.index("    fn call_function(\n")
end = text.index("    fn call_closure(\n", start)
replacement = r'''    fn collect_runtime_type_parameters(ty: &Type, names: &mut BTreeSet<String>) {
        match ty {
    ...
    fn concrete_type_of_value(&self, value: &Value) -> Option<Type> {
        match value {
            ...
            Value::Record { record, .. } => self.program.records.get(record.index()).and_then(
                |definition| {
            ...
            Value::Enum { enumeration, .. } => self.program.enums.get(enumeration.index()).and_then(
                |definition| {
...
# Frozen inspection schemas must reject a type they cannot encode.
...
# Executable success/failure coverage.
Path("crates/nova-cli/tests/fixtures/valid/generic-identity.nv").write_text(
    "fn identity<T>(value: T) -> T { value }\n\n"
    "fn main() -> Int {\n"
    "    if identity(true) { identity(42) } else { 0 }\n"
    "}\n"
)
Path("crates/nova-cli/tests/fixtures/invalid/generic-conflict.nv").write_text(
    "fn first<T>(left: T, right: T) -> T { left }\n"
    "fn main() -> Int { first(1, true) }\n"
)
Path("crates/nova-cli/tests/fixtures/invalid/generic-uninferred.nv").write_text(
    "fn unused<T>() -> Unit { () }\n"
    "fn main() -> Unit { unused() }\n"
)
Path("crates/nova-cli/tests/generic_functions.rs").write_text(
...
# Grammar documentation for the exact surface now implemented.
p = Path("docs/grammar.md")
text = p.read_text()
old = 'function_decl       = "fn" , identifier , "(" , [ parameters ] , ")" , "->" , type_ref , block ;'
assert old in text
p.write_text(
    text.replace(
        old,
...
# Helper artifacts never enter the candidate diff.
for helper in [
    Path(".github/workflows/conquest-bil34.yml"),
    Path(".github/bil34-trigger"),
    Path(".github/bil34_impl.py"),
]:
    if helper.exists():
        helper.unlink()
