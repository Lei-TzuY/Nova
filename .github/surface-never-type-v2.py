...
fn parses_never_in_direct_and_nested_type_positions() {
    ...
    let TypeRefKind::Function { return_type, .. } = &parsed.program.functions[1].parameters[0].ty.kind else { panic!("expected function parameter type"); };
    assert!(matches!(return_type.kind, TypeRefKind::Never));
    let TypeRefKind::Function { parameters, return_type } = &parsed.program.functions[1].return_type.kind else { panic!("expected function return type"); };
...
Path("crates/nova-cli/tests/surface_never_type.rs").write_text(r'''use std::process::Command;
...
fn check_run_and_all_inspection_versions_accept_surface_never() {
    let checked = Command::new(env!("CARGO_BIN_EXE_nova")).args(["check", fixture()]).output().expect("nova check should execute");
    assert!(checked.status.success(), "{}", String::from_utf8_lossy(&checked.stderr));
    let run = Command::new(env!("CARGO_BIN_EXE_nova")).args(["run", fixture()]).output().expect("nova run should execute");
    assert!(run.status.success(), "{}", String::from_utf8_lossy(&run.stderr));
    assert_eq!(String::from_utf8_lossy(&run.stdout), "42\n");
    for version in ["1", "2", "3"] {
        let inspected = Command::new(env!("CARGO_BIN_EXE_nova")).args(["inspect", fixture(), "--format=json", "--schema-version", version]).output().expect("nova inspect should execute");
...
fn every_supported_schema_projects_surface_never_without_a_version_bump() {
    let source = SourceFile::new(SourceId::new(0), "never.nv", "fn forever() -> ! { while true {} } fn main() -> Int { 42 }");
    let lexed = lex(&source);
    let parsed = parse(&source, &lexed.tokens);
    ...
    for program in [&v1_document.program, &v2_document.program, &v3_document.program] {
        let never = program.types.iter().find(|ty| ty.kind == v1::TypeKind::Never).expect("Never type is interned");
